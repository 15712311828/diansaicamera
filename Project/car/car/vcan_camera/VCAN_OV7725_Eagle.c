...
//    port_init(PTA29, ALT1 | IRQ_RISING | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
    ...
    DMA_InitStruct1.majorLoopCnt = CAMERA_DMA_NUM+1;//不知道为什么错位，强行加一个。。。
    ...
    DMA_ITConfig(CAMERA_DMA_CH, kDMA_IT_Major, true);  //允许
    //DMA_EnableRequest(CAMERA_DMA_CH);/* 启动DMA传输 */
...
//      DMA_DADDR(CAMERA_DMA_CH) = (uin32_t)ov7725_eagle_img_buff;    //恢复地址
//      DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
      
     // DMA0->CINT = DMA_CINT_CINT(HW_DMA_CH0);
      
              PORT_InstanceTable[HW_GPIOC]->ISFR = 0xFFFFFFFF;////清除pclk

      DMA_SetDestAddress(CAMERA_DMA_CH,(uint32_t)(ov7725_eagle_img_buff-0x01));//强行减一。。。
...
//    enable_irq(PORTA_IRQn);                         //允许PTA的中断
        PORT_InstanceTable[HW_GPIOB]->ISFR = 0xFFFFFFFF;////清除场中断
